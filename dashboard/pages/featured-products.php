    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <?php foreach ($products as $product): ?>
            <div class="bg-white rounded-lg shadow p-4">
                ...
                <p class="text-gray-600 mb-4">$<?php echo number_format($product['retail_price'], 2); ?></p>
                <form method="POST" class="mt-4">
                    <input type="hidden" name="product_id" value="<?php echo $product['id']; ?>" />
                    <input type="hidden" name="toggle_featured" value="1" />
                    <button type="submit" class="w-full px-4 py-2 rounded-md <?php echo $product['is_featured'] ? 'bg-yellow-500 text-white hover:bg-yellow-600' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'; ?>">
