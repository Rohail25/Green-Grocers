...
// Get package items from JSON field (new schema)
$packageItems = !empty($package['items']) ? json_decode($package['items'], true) : [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || $price <= 0) {
        ...
        // Build items JSON from submitted items
        $itemsPayload = [];
...
<div class="space-y-6">
    ...
    <form method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-sm p-6 space-y-6" id="packageForm">
        ...
        <div>
            ...
            <input type="text" name="package_name" required class="w-full border px-4 py-3 rounded-md" value="<?php echo htmlspecialchars($package['name']); ?>">
        </div>
        ...
        <div>
            ...
            <div id="packageItems" class="space-y-4">
                <?php foreach ($packageItems as $idx => $item): ?>
                    <div class="flex items-start gap-4 bg-gray-50 rounded-lg p-4 border">
                        <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="items[<?php echo $idx; ?>][quantity]" class="w-full border px-3 py-2 rounded-md" value="<?php echo htmlspecialchars($item['quantity']); ?>">
                            </div>
                        </div>
...
<script>
let itemCount = <?php echo count($packageItems); ?>;
