    ...
    if (!$newCategory) {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($_FILES['image']['tmp_name'], $targetPath)) {
                $newImagePath = BASE_PATH . '/public/uploads/products/' . $fileName;
                $images = [$newImagePath];
            }
        }
        ...
        // Try to update with priceUnit, fallback if column doesn't exist
        try {
            $stmt = $conn->prepare("UPDATE products 
                                    SET name = :name, 
                                        categoryId = :categoryId, 
                                        retailPrice = :price, 
                                        totalQuantityInStock = :stock, 
                                        itemSize = :itemSize, 
                                        priceUnit = :priceUnit,
            ...
            // If priceUnit column doesn't exist, update without it
...
// PDO connection is shared; no explicit close needed
?>
...
<div class="space-y-6">
    ...
    <form method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-sm p-6 space-y-6">
            ...
            <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                ...
                <label for="imageUpload" class="cursor-pointer">
                    ...
                    <button type="button" class="px-4 py-2 bg-gray-100 rounded-md hover:bg-gray-200">
                        <?php echo !empty($product['images']) ? 'Change Image' : 'Upload Image'; ?>
                    </button>
                </label>
            </div>
        </div>
        ...
        <div>
            ...
            <input type="text" name="name" required class="w-full border px-4 py-3 rounded-md" value="<?php echo htmlspecialchars($product['name']); ?>">
        </div>
        ...
        <div>
            ...
            <select name="category" required class="w-full border px-4 py-3 rounded-md">
                <?php foreach ($categories as $cat): ?>
                    <option value="<?php echo htmlspecialchars($cat['title']); ?>" <?php echo ($category['title'] === $cat['title']) ? 'selected' : ''; ?>>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <select name="price_unit" required class="w-full border px-4 py-3 rounded-md">
                    ...
                    <option value="Per KG" <?php echo (($product['priceUnit'] ?? '') === 'Per KG') ? 'selected' : ''; ?>>Per KG</option>
                    <option value="Per Each" <?php echo (($product['priceUnit'] ?? '') === 'Per Each') ? 'selected' : ''; ?>>Per Each</option>
                    <option value="Per Dozen" <?php echo (($product['priceUnit'] ?? '') === 'Per Dozen') ? 'selected' : ''; ?>>Per Dozen</option>
                    <option value="Per Pack" <?php echo (($product['priceUnit'] ?? '') === 'Per Pack') ? 'selected' : ''; ?>>Per Pack</option>
                    <option value="Per Liter" <?php echo (($product['priceUnit'] ?? '') === 'Per Liter') ? 'selected' : ''; ?>>Per Liter</option>
                    <option value="Per Piece" <?php echo (($product['priceUnit'] ?? '') === 'Per Piece') ? 'selected' : ''; ?>>Per Piece</option>
                </select>
            </div>
            <div>
        ...
        <div>
            ...
            <textarea name="description" rows="4" class="w-full border px-4 py-3 rounded-md"><?php echo htmlspecialchars($product['description'] ?? ''); ?></textarea>
        </div>
