...
$pageTitle = 'Fresh Vegetables - Green Grocers';

// Get products for this category
if (!empty($searchQuery)) {
    // Search only within Vegetable category
...
$categoryData = [
    ...
    'description' => 'Discover hand-picked, farm-fresh vegetables delivered daily.'
];
?>
...
<div class="w-full">
    ...
    <section class="relative w-full py-12 bg-green-50 mt-[8rem]">
        <div class="max-w-3xl mx-auto px-6 text-center">
            <h1 class="text-3xl md:text-4xl font-bold text-gray-800">Search Results in Vegetables</h1>
        ...
        <div class="max-w-4xl mx-auto">
            <div class="flex gap-2 items-center">
                ...
                <div class="relative flex-1">
                    ...
                    <input type="text" id="vegetable-search-input" placeholder="Search only in Vegetables..." class="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-500" style="font-family: 'Arial', sans-serif;" />
                </div>
                <button onclick="performVegetableSearch()" class="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm font-medium" style="font-family: 'Arial', sans-serif;">Search</button>
            </div>
            
            <!-- Dropdown for search suggestions (only Vegetable category) -->
            <div id="vegetable-search-suggestions" class="mt-2 bg-white border border-gray-300 rounded-lg shadow-lg max-h-96 overflow-y-auto hidden"></div>
        </div>
    </section>
    ...
    <section class="px-6 py-12">
        ...
        <?php if (empty($products)): ?>
            <div class="text-center py-12">
                <p class="text-gray-500 text-lg">No vegetables found<?php echo !empty($searchQuery) ? ' for "' . htmlspecialchars($searchQuery) . '"' : ''; ?>.</p>
...
let vegetableSearchDebounce;
document.addEventListener('DOMContentLoaded', function() {
    const searchInput = document.getElementById('vegetable-search-input');
    const suggestionDropdown = document.getElementById('vegetable-search-suggestions');
    
    if (searchInput) {
        searchInput.addEventListener('input', function() {
            clearTimeout(vegetableSearchDebounce);
            ...
            vegetableSearchDebounce = setTimeout(() => {
                // Fetch suggestions for Vegetable category only
                fetch('<?php echo BASE_PATH; ?>/includes/search-suggestions.php?q=' + encodeURIComponent(query) + '&category=Vegetable&limit=8')
        ...
        document.addEventListener('click', function(e) {
            if (!e.target.closest('#vegetable-search-input') && !e.target.closest('#vegetable-search-suggestions')) {
                suggestionDropdown.classList.add('hidden');
            }
        });
    }
...
function performVegetableSearch() {
    const searchInput = document.getElementById('vegetable-search-input');
    const query = searchInput ? searchInput.value.trim() : '';
    if (query) {
        window.location.href = '<?php echo BASE_PATH; ?>/vegetable?search=' + encodeURIComponent(query);
    }
}
