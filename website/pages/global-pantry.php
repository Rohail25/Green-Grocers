...
$categoryName = 'Global Pantry';
$searchQuery = isset($_GET['search']) ? trim($_GET['search']) : '';
$pageTitle = 'Global Pantry - Green Grocers';

// Get products for this category
if (!empty($searchQuery)) {
    // Search only within Global Pantry category
    $products = searchProductsByCategory($searchQuery, $categoryName);
} else {
    // Get all global pantry products
    $products = getProductsByCategory($categoryName);
}

$categoryData = [
    'heading' => 'Global Pantry',
    'description' => 'Premium pantry essentials sourced from around the world.'
];
?>
...
<div class="w-full">
    ...
    <section class="relative w-full py-12 bg-green-50 mt-[6rem]">
        <div class="max-w-3xl mx-auto px-6 text-center">
            <h1 class="text-3xl md:text-4xl font-bold text-gray-800">Search Results in Global Pantry</h1>
        ...
        <div class="max-w-4xl mx-auto">
            <div class="flex gap-2 items-center">
                <div class="relative flex-1">
                    ...
                    <input type="text" id="pantry-search-input" placeholder="Search only in Global Pantry..." class="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-500" style="font-family: 'Arial', sans-serif;" />
                </div>
                <button onclick="performPantrySearch()" class="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm font-medium" style="font-family: 'Arial', sans-serif;">Search</button>
            </div>
            
            <!-- Dropdown for search suggestions (only Global Pantry category) -->
            <div id="pantry-search-suggestions" class="mt-2 bg-white border border-gray-300 rounded-lg shadow-lg max-h-96 overflow-y-auto hidden"></div>
        </div>
    </section>
...
let pantrySearchDebounce;
document.addEventListener('DOMContentLoaded', function() {
    const searchInput = document.getElementById('pantry-search-input');
    const suggestionDropdown = document.getElementById('pantry-search-suggestions');
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function() {
            ...
            pantrySearchDebounce = setTimeout(() => {
                // Fetch suggestions for Global Pantry category only
                fetch('<?php echo BASE_PATH; ?>/includes/search-suggestions.php?q=' + encodeURIComponent(query) + '&category=Global%20Pantry&limit=8')
        ...
        document.addEventListener('click', function(e) {
            if (!e.target.closest('#pantry-search-input') && !e.target.closest('#pantry-search-suggestions')) {
                suggestionDropdown.classList.add('hidden');
            }
        });
    }
...
function performPantrySearch() {
    ...
    if (query) {
        window.location.href = '<?php echo BASE_PATH; ?>/website/pages/global-pantry.php?search=' + encodeURIComponent(query);
    }
}
